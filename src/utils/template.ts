...
export function generateContentFromTemplate(
    ...
    const template = Handlebars.compile(setting.message_template);
...
export function generatePath(msg: MessageUpdate, setting: TGInboxSettings): string {
    const data = sanitizePathData(buildPathData(msg));
    const template = Handlebars.compile(setting.custom_file_path);
    return template(data);
}
...
function formatPrivateChatName(chat: { first_name?: string; last_name?: string }): string {
