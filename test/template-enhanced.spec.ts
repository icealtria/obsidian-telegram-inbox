...
function createMockMessage(text: string, overrides?: Partial<MessageUpdate>): MessageUpdate {
    return {
        message_id: 1,
        from: {
            id: 123,
...
describe("generateContentFromTemplate", () => {
    ...
    test("handles template with all available fields", () => {
        ...
        const result = generateContentFromTemplate(msg, settings);
        assert.strictEqual(result, "Test | Test User | testuser | 123 | 2021-07-21 | 14:00 | 1");
    });
...
describe("buildMsgData", () => {
    test("builds message data for normal message", () => {
        ...
        assertObjectContains(result as unknown as Record<string, unknown>, {
            message_id: 1,
            text: "Hello world",
            date: "2021-07-21",
            time: "14:00",
