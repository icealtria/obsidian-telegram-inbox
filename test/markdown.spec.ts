...
const baseSettings: TGInboxSettings = { ...DEFAULT_SETTINGS, disable_auto_reception: true };
...
describe("toMarkdownV2", () => {
    ...
    test("converts bold text", () => {
        const msg = createMockMessage("Hello bold world", [{ type: "bold", offset: 6, length: 4 }]);
        const result = toMarkdownV2(msg, baseSettings);
    ...
    test("converts inline code", () => {
        const msg = createMockMessage("Hello code world", [{ type: "code", offset: 6, length: 4 }]);
        const result = toMarkdownV2(msg, baseSettings);
